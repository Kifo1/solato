...
export function AnalyticCalendar() {
  ...
  const { weeks, monthLabels, maxSeconds, activeDays, totalSeconds } =
    useMemo(() => {
    ...
    <div className="rounded-xl border border-slate-200/10 bg-slate-200/5 p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant={"secondary"}
            className="border border-white/20 rounded-xl p-2!"
      ...
      {tooltip &&
        createPortal(
          <div
            className="pointer-events-none fixed z-50 -translate-x-1/2 -translate-y-full rounded-lg border border-slate-200/10 bg-slate-900 px-3 py-2 text-xs text-white shadow-lg whitespace-nowrap"
            style={{ left: tooltip.x, top: tooltip.y - 8 }}
          >
            <p className="font-semibold">
              {formatSecondsToString(tooltip.seconds)}
            </p>
            <p className="text-blue-200/70">
              {format(tooltip.date, "EEEE, MMM d, yyyy")}
            </p>
          </div>,
          document.body,
        )}
        ...
        <div className="inline-flex gap-3">
          <div className="flex flex-col gap-1 pt-5">
            {WEEKDAY_LABELS.map((label, i) => (
              <span
                key={i}
                className="h-3 md:h-3.5 text-[10px] leading-3 text-blue-200/60"
              >
                {label}
              </span>
            ))}
                ...
                <div key={weekIndex} className="flex flex-col gap-1">
                  {week.map((day) => (
                    <div
                      ...
                      className={`w-3 h-3 md:w-3.5 md:h-3.5 rounded-sm transition-transform ${
                        day.inYear
                          ? `${LEVEL_COLORS[getLevel(day.seconds, maxSeconds)]} hover:scale-125 cursor-pointer`
                          : "opacity-0"
      ...
      <div className="flex items-center justify-end gap-1.5 mt-4">
        <span className="text-[10px] text-blue-200/60 mr-1">Less</span>
        {LEVEL_COLORS.map((color, i) => (
          <div key={i} className={`w-3 h-3 rounded-sm ${color}`} />
        ))}
        <span className="text-[10px] text-blue-200/60 ml-1">More</span>
      </div>
    </div>
  );
}
